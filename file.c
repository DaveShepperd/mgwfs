				   ...
				   loff_t *ppos)
{
	...
	if ( !ourSuper || verbose )
	{
		pr_info("mgwfs_read(): file '%s': sb=%p, ourInode=%p, ourSuper=%p, *ppos=%lld, len=%ld, size=%d\n",
				ourFileName, sb, ourInode, ourSuper, *ppos, len, ourInode->fsHeader.size);
	}
	if ( *ppos >= ourInode->fsHeader.size || !len )
	{
		return 0;
	}
	...
	retIdx = 0;
	retPtr = ourInode->fsHeader.pointers[0];         /* pointer to list of retrieval pointers */
	if ( verbose )
	{
	...
	while ( bytesRead < len && retIdx < FSYS_MAX_FHPTRS )
	{
		...
		if ( bytesToCopy + *ppos > ourInode->fsHeader.size ) /* then clip again if to read beyond EOF */
			bytesToCopy = ourInode->fsHeader.size - *ppos;
		if ( bytesToCopy <= 0 )
		{
			pr_err("mgwfs_read(): file '%s' computed bytesToCopy as 0. len=%ld, bytesRead=%d, numBytes=%d, relativeSector=%d, nblocks=%d, *ppos=%lld, size=%d\n",
				   ourFileName, len, bytesRead, numBytes, relativeSector, retPtr->nblocks, *ppos, ourInode->fsHeader.size);
			break;
		}
		if ( verbose )
		{
		...
		buf += bytesToCopy;
		*ppos += bytesToCopy;
		if ( *ppos >= ourInode->fsHeader.size )
			break;
...
loff_t mgwfs_llseek(struct file *filp, loff_t offset, int whence)
{
	...
	{
	case SEEK_SET:
		if ( (offset > ourInode->fsHeader.size) || (offset < 0) )
		{
			if ( verbose )
			{
				pr_info("mgwfs_llseek(): File '%s'. Seek SET out of range: offset=%lld is >= %d or < 0\n",
						ourFileName, offset, ourInode->fsHeader.size);
			}
			return -EINVAL;
		}
		filp->f_pos = offset;
	...
	case SEEK_CUR:
		...
		if ( (temp > ourInode->fsHeader.size) || (temp < 0) )
		{
			if ( verbose )
			{
				pr_info("mgwfs_llseek(): File '%s'. Seek CUR out of range: %lld%+lld(%lld) >= %d or < 0\n",
						ourFileName, filp->f_pos, offset, temp, ourInode->fsHeader.size);
			}
			return -EINVAL;
		}
		filp->f_pos = temp;
		break;
	case SEEK_END:
		temp = ourInode->fsHeader.size + offset;
		if ( (temp > ourInode->fsHeader.size) || (temp < 0) )
		{
			if ( verbose )
			{
				pr_info("mgwfs_llseek(): File '%s'. Seek END out of range: %d%+lld(%lld) >= %d or < 0\n",
						ourFileName, ourInode->fsHeader.size, offset, temp, ourInode->fsHeader.size);
			}
			return -EINVAL;
		}
		filp->f_pos = temp;
